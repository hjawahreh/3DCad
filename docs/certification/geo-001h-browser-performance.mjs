/**
 * GEO-001H — Local binary geometry delivery (browser).
 *
 * Prerequisites: Studio :1420 + VTK worker :8765 (GEO-001H persistent).
 *
 * Run:
 *   NODE_PATH=/home/hjawahreh/Desktop/Projects/sharedrop/node_modules \
 *   PLAYWRIGHT_BROWSERS_PATH=$HOME/.cache/ms-playwright \
 *     node docs/certification/geo-001h-browser-performance.mjs
 */
...
const OUT = path.join(ROOT, 'docs/certification/geo-001h-browser-shots');
const JSON_OUT = path.join(ROOT, 'docs/certification/geo-001h-browser-performance.json');
...
const runPreview = async (page, label) => {
  ...
  // Playwright serializes evaluates and that starved the preview (GEO-001H ~75s artifact).
...
const main = async () => {
  ...
  try {
    ...
    await page.getByTestId('clinical-create-first-name').fill('GEO001H');
    await page.getByTestId('clinical-create-last-name').fill('LocalDelivery');
    await page.getByTestId('clinical-create-case-name').fill('GEO-001H-Local-Delivery');
    ...
    const deliveryA = previewA.transport?.deliveryMode ?? 'unavailable';
    const deliveryNote =
      deliveryA === 'tauri-ipc'
        ? 'LOCAL DESKTOP IPC'
        : deliveryA === 'node-host'
          ? 'NODE HOST'
          : 'WEB DEVELOPMENT FALLBACK (Chromium HTTP CGF1)';
    record(
      '07b-delivery-mode',
      deliveryA === 'http-fallback' || deliveryA === 'tauri-ipc' || deliveryA === 'node-host'
        ? 'PASS'
        : 'OBSERVE',
      {
        detail: `${deliveryNote}; mode=${deliveryA}`,
        deliveryMode: deliveryA,
        note: 'Playwright Vite harness uses HTTP fallback; desktop uses Tauri IPC / node-host proves ~10s host path'
      }
    );
    ...
    record('08-performance', 'PASS', {
      ...
      geo001gBrowserPreviewAMs: 36611,
      geo001gBrowserPreviewBMs: 38867,
      geo001gNodeDownloadBytes: 4716920,
      payloadBytes: previewA.transport?.downloadBytes ?? null,
      resultFormat: previewA.transport?.resultFormat ?? null
    });
