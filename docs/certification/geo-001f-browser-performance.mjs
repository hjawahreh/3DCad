/**
 * GEO-001F — Persistent worker Trim latency (browser).
 *
 * Prerequisites: Studio :1420 + VTK worker :8765 (GEO-001F persistent).
 *
 * Run:
 *   NODE_PATH=/home/hjawahreh/Desktop/Projects/sharedrop/node_modules \
 *   PLAYWRIGHT_BROWSERS_PATH=$HOME/.cache/ms-playwright \
 *     node docs/certification/geo-001f-browser-performance.mjs
 */
...
const OUT = path.join(ROOT, 'docs/certification/geo-001f-browser-shots');
const JSON_OUT = path.join(ROOT, 'docs/certification/geo-001f-browser-performance.json');
...
const runPreview = async (page, label) => {
  ...
  // Playwright serializes evaluates and that starved the preview (GEO-001F ~75s artifact).
...
const main = async () => {
  ...
  try {
    ...
    await page.getByTestId('clinical-create-first-name').fill('GEO001F');
    await page.getByTestId('clinical-create-last-name').fill('PersistWorker');
    await page.getByTestId('clinical-create-case-name').fill('GEO-001F-Persistent-Worker');
    ...
    await shot(page, '06-after-accept-b');
    record(
      '03-preview-b-accept',
    ...
    record('07-performance', 'PASS', {
      detail: Object.entries(evidence.timings)
        .filter(([, v]) => typeof v === 'number')
        .map(([k, v]) => `${k}=${v}ms`)
        .join(', '),
      timings: evidence.timings,
      geo001eBaselinePreviewMs: 49584,
      nodeWorkerTrimMs: { trim1: 8642, trim2: 10666, ensureMs: 187 }
    });
  } catch (err) {
