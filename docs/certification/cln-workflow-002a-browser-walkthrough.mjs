/**
 * CLN-WORKFLOW-002A — final manual-gate closure browser walkthrough.
 *
 ...
 *     node docs/certification/cln-workflow-002a-browser-walkthrough.mjs
 */
...
const OUT = path.join(ROOT, 'docs/certification/cln-workflow-002a-browser-shots');
const JSON_OUT = path.join(ROOT, 'docs/certification/cln-workflow-002a-browser-walkthrough.json');
...
const main = async () => {
  ...
  try {
    ...
    await shot(page, '04-trim-result');
    ...
    await shot(page, '06-upper-base');

    // Lower base
    await page.getByTestId('clinical-close-base-arch-lower').click();
    await waitIdle(page, 800);
    await page.evaluate(() => {
      globalThis.__clinicalWorkspace?.closeBase?.setActiveArch?.('lower');
      globalThis.__clinicalWorkspace?.session?.notifyUi?.();
    });
    await waitBaseIdle(page);
    ...
    await shot(page, '07-lower-base');

    await page.getByTestId('clinical-close-base-done').click({ timeout: 60000 }).catch(async () => {
      await page.evaluate(() => {
        const ws = globalThis.__clinicalWorkspace;
        ws?.closeBase?.cancel?.();
    ...
    record('18-base-done-opens-segment', 'PASS');
    ...
    record('19-mark-teeth-contract', 'PASS', {
