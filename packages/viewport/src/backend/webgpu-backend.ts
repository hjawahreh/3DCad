...
export class WebGpuBackend implements GpuBackend {
  ...
  private threeRenderer: ThreeWebGpuRenderer | undefined;
  ...
  public async initialize(
    ...
    if (this.disposed) {
      return renderFailure('unavailable', 'WebGPU backend is disposed.');
    }
    this.canvas = canvas;
    ...
    if (canvas !== undefined && hasGpu) {
      try {
        ...
        if (typeof mod.WebGPURenderer === 'function') {
    ...
    // Capability-faithful stub when Three/WebGPU is unavailable.
  ...
  public createPipeline(info: PipelineCreateInfo): RenderResult<GpuHandle> {
    ...
    if (!this.resources.has(Number(info.shaderHandle))) {
      // Allow pipelines with opaque external shader handles in stub mode.
      if (!this.stubMode) {
        return renderFailure('not-found', 'Shader handle does not exist.');
      }
      this.allocate('shader', 0);
    }
    const handle = this.allocate('pipeline', 128, info.label);
    return renderSuccess(asGpuHandle(handle));
  }
  ...
  private requireReady(): RenderResult<void> {
    ...
    if (!this.initialized) {
      return renderFailure('unavailable', 'WebGPU backend is not initialized.');
    }
    return renderSuccess(undefined);
  }
